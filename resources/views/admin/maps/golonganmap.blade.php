            ...
            <div class="card-body">
                <div id="map-wrapper">
                    ...
                    <div class="gmap-info" id="infoPanel">
                        <h4>Golongan Narkoba per Desa</h4>
                        <div id="infoContent">Arahkan kursor ke suatu Desa</div>
                    </div>

                    <div class="gmap-legend">
                        <h4>Keterangan</h4>
                        <div><span style="background:#FF0000"></span> Golongan I</div>
                        <div><span style="background:#FFFF00"></span> Golongan II</div>
                        <div><span style="background:#808080"></span> Golongan III</div>
                        <div><span style="background:#0000FF"></span> Golongan I &amp; II</div>
                        <div><span style="background:#FFA500"></span> Golongan II &amp; III</div>
                        <div><span style="background:#8B4513"></span> Golongan I &amp; III</div>
                        <div><span style="background:#000000"></span> Golongan I, II &amp; III</div>
                        <div><span style="background:#0B6623"></span> Tidak Ada Data</div>
                    </div>
...
function initMap() {
    ...
    function getColor(golongan) {
        if (!golongan || golongan === '' || golongan === null) return '#0B6623';
        var gol = golongan.replace(/\s/g, '').split(',');
        var hasI = gol.indexOf('GolonganI') !== -1;
        var hasII = gol.indexOf('GolonganII') !== -1;
        var hasIII = gol.indexOf('GolonganIII') !== -1;
        if (hasI && hasII && hasIII) return '#000000';
        if (hasI && hasII) return '#0000FF';
        if (hasII && hasIII) return '#FFA500';
        if (hasI && hasIII) return '#8B4513';
        if (hasI) return '#FF0000';
        if (hasII) return '#FFFF00';
        if (hasIII) return '#808080';
        return '#0B6623';
    }

    function applyDefaultStyle() {
        map.data.setStyle(function(feature) {
            var gol = feature.getProperty('golongan_positif');
            return {
                fillColor: getColor(gol),
        ...
        .map(function(d) {
            return {
                ...
                properties: {
                    name: d.nama_desa,
                    id: d.id,
                    golongan_positif: d.golongan_positif || null,
    ...
    map.data.addListener('mouseover', function(event) {
        ...
        var gol = event.feature.getProperty('golongan_positif');
        document.getElementById('infoPanel').innerHTML =
            '<h4>Golongan Narkoba per Desa</h4><div id="infoContent"><b>' + name + '</b><br>' +
            (gol ? gol : 'Tidak Ada Data') + '</div>';
    });
    map.data.addListener('mouseout', function(event) {
        map.data.revertStyle(event.feature);
        document.getElementById('infoPanel').innerHTML =
            '<h4>Golongan Narkoba per Desa</h4><div id="infoContent">Arahkan kursor ke suatu Desa</div>';
    });
    ...
    opacitySlider.addEventListener('input', function() {
        ...
        map.data.setStyle(function(feature) {
            var gol = feature.getProperty('golongan_positif');
            return { fillColor: getColor(gol), fillOpacity: currentOpacity, strokeColor: '#ffffff', strokeWeight: 2, strokeOpacity: 1, visible: true };
        });
    });
